...
for (variable in binary_variables) {
  plot <- plot_churn_distribution(training, variable)
...
category_data$Churn <- as.factor(category_data$Churn)

# Create the combined plot using the melted data
ggplot(category_data, aes(x = value, fill = Churn)) + 
  geom_bar(position = "fill") +
  scale_fill_manual(values = c("0" = "darkolivegreen3", "1" = "brown3")) +
...
```{r}
model_QDA <- qda(Churn~., data=X_train)
model_QDA
```
