...
post_prob_QDA <- predictions_QDA$posterior
head(post_prob_QDA)
# QDA into labels
predictions_QDA_label <- predictions_QDA$class 
head(predictions_QDA_label)

# Scatterplot with predictions: good->red and bad->black
colors_QDA_good_bad <- c("black","red")[1*(testing_transf[,5]==predictions_QDA_label)+1]
...
confMat_QDA <- table(predictions_QDA$class, testing_transf$Churn)
...
pairs(testing_transf[,1:4],
      main="Bad (in black) classifications with LDA",
      pch=19,
...
# Performance measures
confMat_LDA <- table(predictions_LDA$class, y_test)
...
```{r}
# Train Naïve Bayes model -> laplace controls smoothing of probabilities
...
# Evaluate the model
confMat_NB <- table(Predicted = predictions_NB, Actual = y_test)
...
```{r}
# Naïve Bayes with Caret
library(e1071)

# Set up train control -> 5 repeats of 10-fold cross validation
trctrl_NB <- trainControl(method = "repeatedcv",
                          repeats = 5,
                          number = 10,
...
trctrl_LR <- trainControl(method = "repeatedcv",
                             repeats = 1,
                             number = 5)
# Train model
model_LR <- train(x = X_train, 
                  y = y_train,
                  method = "glmnet",
                  family = "binomial",
