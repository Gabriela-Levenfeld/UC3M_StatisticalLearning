...
```{r initial_kknModel}
# Initial model: Confusion matrix (threshold 0.5)
# Predict probabilities on the test set
pre_initial_mod <- predict(model_knnFit, newdata = testing_transf)
confusionMatrix(pre_initial_mod, testing_transf$class)
```
...
```{r roc_plots, echo=FALSE, warning=FALSE, message=FALSE, fig.align='center'}
# ROC for each model
...
roc_data <- data.frame() # Empty df

# Iterate over each model and bind the data
for (model_name in names(roc_list)) {
  model_roc <- roc_list[[model_name]]
  model_df <- data.frame(
    Specificity = 1 - model_roc$specificities,
    Sensitivity = model_roc$sensitivities,
    Model = model_name
  )
  roc_data <- rbind(roc_data, model_df)
}

# Convert into factor model column
roc_data$Model <- factor(roc_data$Model)

# Plot ROC curves
fig <- plot_ly(roc_data, x = ~Specificity, y = ~Sensitivity, color = ~Model, type = 'scatter', mode = 'lines') %>%
